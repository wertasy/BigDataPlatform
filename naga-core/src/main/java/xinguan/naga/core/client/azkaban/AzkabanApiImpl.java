package xinguan.naga.core.client.azkaban;
...
public class AzkabanApiImpl implements AzkabanApi {
  ...
  private static final String FETCH_PROJECT_FLOWS =
      "{0}/manager?ajax=fetchprojectflows&session.id={1}&project={2}";
  private static final String FETCH_ALL_PROJECTS =
      "{0}/index?ajax=fetchuserprojects&session.id={1}";
  private static final String CREATE_PROJECT = "{0}/manager";
  private static final String EXECUTE_FLOW =
      "{0}/executor?ajax=executeFlow&session.id={1}&project={2}&flow={3}";
  private static final String CANCEL_FLOW =
      "{0}/executor?ajax=cancelFlow&session.id={1}&execid={2}";
  private static final String FETCH_EXEC_FLOW =
      "{0}/executor?ajax=fetchexecflow&session.id={1}&execid={2}";
  private static final String FETCH_EXEC_JOB_LOGS =
      "{0}/executor?ajax=fetchExecJobLogs&session.id={1}&execid={2}"
          + "&jobId={3}&offset={4}&length={5}";
  private static final String FETCH_FLOW_EXECUTIONS =
      "{0}/manager?ajax=fetchFlowExecutions&session.id={1}"
          + "&project={2}&flow={3}&start={4}&length={5}";
  private static final String SCHEDULE_CRON_FLOW =
      "{0}/schedule?ajax=scheduleCronFlow&session.id={1}&"
          + "projectName={2}&flow={3}&cronExpression={4}";
  private static final String FETCH_SCHEDULE =
      "{0}/schedule?ajax=fetchSchedule&session.id={1}&projectId={2}&flowId={3}";
  ...
  public BaseResponse createProject(String name, String desc) {
    Request res =
        Request.Post(MessageFormat.format(CREATE_PROJECT, uri))
            .bodyForm(
                Form.form()
  ...
  public FetchFlowsResponse fetchProjectFlows(String projectName) {
    Request res =
        Request.Get(MessageFormat.format(FETCH_PROJECT_FLOWS, uri, sessionId, projectName));
  ...
  public ExecFlowResponse executeFlow(String projectName, String flowName) {
    Request res =
        Request.Post(MessageFormat.format(EXECUTE_FLOW, uri, sessionId, projectName, flowName));
  ...
  @Override
  public FetchFlowExecutionsResponse fetchFlowExecutions(
      String projectName, String flowName, int start, int length) {
    Request res =
        Request.Get(
            MessageFormat.format(
                FETCH_FLOW_EXECUTIONS,
                uri,
                sessionId,
                projectName,
                flowName,
  ...
  @Override
  public ScheduleCronFlowResponse scheduleCronFlow(
      String projectName, String flowName, String cronExpression)
    ...
    Request res =
        Request.Post(
            MessageFormat.format(
                ...
                projectName,
                flowName,
                URLEncoder.encode(cronExpression, "UTF-8")));
  ...
  public FetchScheduleResponse fetchSchedule(String projectId, String flowId) {
    Request res =
        Request.Get(MessageFormat.format(FETCH_SCHEDULE, uri, sessionId, projectId, flowId));
  ...
  public BaseResponse removeSchedule(String scheduleId) {
    Request res =
        Request.Post(MessageFormat.format(REMOVE_SCHEDULE, uri))
            .bodyForm(
                Form.form()
                    .add("session.id", sessionId)
