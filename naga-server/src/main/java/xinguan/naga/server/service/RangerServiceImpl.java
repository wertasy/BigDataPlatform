...
import xinguan.naga.core.client.ranger.RangerClient;
import xinguan.naga.core.client.ranger.config.RangerAuthConfig;
import xinguan.naga.core.client.ranger.config.RangerClientConfig;
import xinguan.naga.core.client.ranger.model.*;
import xinguan.naga.entity.privilege.HdfsResource;
import xinguan.naga.entity.privilege.HiveResource;
...
public class RangerServiceImpl implements RangerService {
  ...
  @Value("${custom.ranger.url}")
  private String rangerUrl;

  @Value("${custom.ranger.user}")
  private String rangerUser;

  @Value("${custom.ranger.pwd}")
  private String rangerPwd;

  @Value("${custom.ranger.service.hdfs}")
  private String rangerHdfs;

  @Value("${custom.ranger.service.hive}")
  private String rangerHive;
  ...
  @Override
  public void removeRangerUser(String name) {
  ...
  @Override
  public void addPolicy(ResourcePrivilege resourcePrivilege) {
    ...
    if (resourcePrivilege.getResourceType().equals(ResourceType.HDFS)) {
      ...
      PolicyResource policyResource =
          PolicyResource.builder()
              .values(Arrays.asList(hdfsResource.getPath()))
              .isRecursive(hdfsResource.getRecursive())
              .build();
      ...
      hdfsResource
          ...
          .forEach(
              type -> {
                policyItemAccessList.add(
                    PolicyItemAccess.builder().type(type).isAllowed(true).build());
              });
      ...
      policy =
          Policy.builder()
              .service(rangerHdfs)
              .name(resourcePrivilege.getName())
              .isEnabled(true)
              .policyType(0)
              .resources(policyResourceMap)
              .policyItems(Arrays.asList(policyItem))
              .build();
