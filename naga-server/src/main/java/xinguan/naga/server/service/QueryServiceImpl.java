...
import xinguan.naga.repository.query.SavedSqlRepository;
import xinguan.naga.server.query.ConnectionPool;
import xinguan.naga.server.query.DataCacheUtil;
import xinguan.naga.server.query.JdbcDao;
import xinguan.naga.server.query.QueryObject;
import xinguan.naga.server.query.cache.DataFrameCache;
import xinguan.naga.server.query.dataframe.ColumnInfo;
import xinguan.naga.server.query.dataframe.DataFrame;
import xinguan.naga.server.query.dataframe.PagedDataFrame;
...
public class QueryServiceImpl implements QueryService {
  @Autowired SavedSqlRepository savedSqlRepository;

  @Override
  public Map<String, Object> executeQuery(QueryObject queryObject, int pageIndex)
  ...
  @Override
  public DataFrame executeDataFrame(QueryObject queryObject, int pageIndex)
      throws ExecutionException, InterruptedException {
    // 获取hexkey
    String hexkey =
        DataCacheUtil.generateMd5(
            DataCacheUtil.getUnionStr(queryObject.getCurrentUser(), queryObject.getSql()));
    ...
    if (pagedDataFrame.getRowSchemaInfo() == null) {
        ...
        Connection connection =
            ConnectionPool.getConnection(
                Joiner.on("|")
                    .join(
                        Arrays.asList(
                            queryObject.getCurrentUser(), queryObject.getEngineType().name())));
        try {
          dataFrame = JdbcDao.queryAsDataFrame(queryObject, connection);
          break;
        } catch (NagaException e) {
          e.printStackTrace();
      ...
      if (pagedDataFrame.getNowPageIndex() == pageIndex
          && pagedDataFrame.getNowPageSize() == queryObject.getPageSize()) {
  ...
  @Override
  public Map<String, String> getTableInfo(QueryObject queryObject)
