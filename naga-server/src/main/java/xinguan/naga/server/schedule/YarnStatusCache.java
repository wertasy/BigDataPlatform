...
public class YarnStatusCache {
  ...
  private String APPS_URI_FORMAT =
      "http://%s/ws/v1/cluster/apps?states=%s&limit=%s&deSelects" + "=resourceRequests";

  private LoadingCache<String, Object> yarnRealTimeInfoLoadingCache;

  @PostConstruct
  public void init() {
    yarnRealTimeInfoLoadingCache =
                    ...
                    switch (split[0].toLowerCase()) {
                      case "scheduler":
                        try {
                          ...
                          String body =
                              HttpUtil.createGet(schedulerUri)
                                  .header(Header.ACCEPT, ContentType.JSON.toString())
                                  .execute()
                                  .body();
                      ...
                      case "apps":
                        try {
                          String appsUri =
                              String.format(APPS_URI_FORMAT, activeRmUri, split[1], split[2]);
                          String appBody =
                              HttpUtil.createGet(appsUri)
                                  .header(Header.ACCEPT, ContentType.JSON.toString())
                                  .execute()
                                  .body();
    ...
    if (rmUris.size() > 1) {
      for (String uri : rmUris) {
        try {
          ...
          String rmBody =
              HttpUtil.createGet(String.format("http://%s/ws/v1/cluster/info", uri))
                  .header(Header.ACCEPT, ContentType.JSON.toString())
                  .execute()
                  .body();
