package xinguan.naga.server.schedule;

import cn.hutool.core.date.DateUtil;
import xinguan.naga.core.util.StatefulHttpClient;
...
public class HadoopJmxSchedule {
  ...
  public static final String QUEUEMETRICS =
  ...
  private String getActiveRmUri(List<String> rmUris) throws IOException {
    ...
    if (rmUris.size() > 1) {
      for (String uri : rmUris) {
        ...
        HadoopMetrics hadoopMetrics =
            client.get(HadoopMetrics.class, clusterMetricsUrl, null, null);
        if (hadoopMetrics
            .getMetricsValue("tag.ClusterMetrics")
            .toString()
  ...
  private YarnSummary reportYarnSummary(StatefulHttpClient client) {
    ...
    try {
      String clusterMetricsUrl =
          String.format(JMXSERVERURLFORMAT, getActiveRmUri(rmUris), CLUSTERMETRICS);
      HadoopMetrics clusterMetrics = client.get(HadoopMetrics.class, clusterMetricsUrl, null, null);
      if (clusterMetrics
          .getMetricsValue("tag.ClusterMetrics")
          .toString()
          .equals("ResourceManager")) {
        yarnSummary.setLiveNodeManagerNums((int) clusterMetrics.getMetricsValue("NumActiveNMs"));
        yarnSummary.setDeadNodeManagerNums((int) clusterMetrics.getMetricsValue("NumLostNMs"));
        yarnSummary.setUnhealthyNodeManagerNums(
            (int) clusterMetrics.getMetricsValue("NumUnhealthyNMs"));

        String queueMetricsUrl =
            String.format(JMXSERVERURLFORMAT, getActiveRmUri(rmUris), QUEUEMETRICS);
        HadoopMetrics hadoopMetrics = client.get(HadoopMetrics.class, queueMetricsUrl, null, null);
        ...
        yarnSummary.setAllocatedContainers(
            (int) hadoopMetrics.getMetricsValue("AllocatedContainers"));
        ...
        yarnSummary.setPendingContainers((int) hadoopMetrics.getMetricsValue("PendingContainers"));
        yarnSummary.setReservedMem(
        ...
        yarnSummary.setReservedContainers(
            (int) hadoopMetrics.getMetricsValue("ReservedContainers"));
  ...
  private HdfsSummary reportHdfsSummary(StatefulHttpClient client) throws IOException {
    ...
    try {
      ...
      HadoopMetrics hadoopMetrics = client.get(HadoopMetrics.class, nameNodeInfoUrl, null, null);
      ...
      hdfsSummary.setDfsFree(Long.parseLong(hadoopMetrics.getMetricsValue("Free").toString()));
      ...
      hdfsSummary.setTotalFiles(
          Long.parseLong(hadoopMetrics.getMetricsValue("TotalFiles").toString()));
      hdfsSummary.setMissingBlocks(
          Long.parseLong(hadoopMetrics.getMetricsValue("NumberOfMissingBlocks").toString()));

      String fsNameSystemStateUrl =
          String.format(JMXSERVERURLFORMAT, activeNameNodeUri, FSNAMESYSTEMSTATE);
      HadoopMetrics fsNameSystemMetrics =
          client.get(HadoopMetrics.class, fsNameSystemStateUrl, null, null);
      hdfsSummary.setLiveDataNodeNums(
          (int) fsNameSystemMetrics.getMetricsValue("NumLiveDataNodes"));
      hdfsSummary.setDeadDataNodeNums(
          (int) fsNameSystemMetrics.getMetricsValue("NumDeadDataNodes"));
      hdfsSummary.setVolumeFailuresTotal(
          (int) fsNameSystemMetrics.getMetricsValue("VolumeFailuresTotal"));
    } catch (Exception e) {
      log.error(e.getMessage());
  ...
  private List<QueueMetrics> queryQueueMetrics() throws IOException {
    ...
    String queueMetricsUrl =
        String.format(JMXSERVERURLFORMAT, getActiveRmUri(rmUris), QUEUEMETRICSALL);
    HadoopMetrics clusterMetrics = client.get(HadoopMetrics.class, queueMetricsUrl, null, null);
