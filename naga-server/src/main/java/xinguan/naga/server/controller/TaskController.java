...
import xinguan.naga.server.service.TaskService;
...
public class TaskController extends BaseController {
  ...
  @Autowired PluginService pluginManager;

  @Autowired TaskService taskManager;
  ...
  public Object createTask(@RequestBody @OperationObj TaskGraph taskGraph) throws Exception {
    ...
    if (!matcher.matches()) {
      return asErrorResponse(ErrorCodes.ERROR_PARAM, "task name can only contains a-zA-Z0-9-_");
    }
  ...
  private TaskWorkFlow buildTaskInfo(TaskGraph taskGraph) {
    ...
    taskInfo.setCreateTime(DateUtil.toIntSecond(new Date()));
    ...
    for (TaskGraph.TaskNode taskNode : taskGraph.getNodeList()) {
      ...
      jobInfo.setTrash(false);
      jobInfo.setCreateTime(DateUtil.toIntSecond(new Date()));
      ...
      if (pkg == null) {
        if (jobInfoInDb == null) {
          throw new NagaException("pkg and job info not found", ErrorCodes.ERROR_PARAM);
        }
        ...
        taskNode
            ...
            .forEach(
                stringObjectMap -> {
                  jobInfoInDb
                      .getParams()
                      .put(stringObjectMap.get("name").toString(), stringObjectMap.get("value"));
                });
  ...
  private TaskGraph buildTaskGraph(TaskWorkFlow taskInfo) {
    ...
    List<TaskGraph.TaskNode> nodes =
        taskInfo.getJobInfoList().stream()
            .map(
                new Function<JobInfo, TaskGraph.TaskNode>() {
                  @Override
  ...
  public List<Map<String, Object>> filterOutInternalParams(JobInfo jobInfo) {
    ...
    jobInfo.getParams().keySet().stream()
        .forEach(
            new Consumer<String>() {
              @Override
              public void accept(String s) {
                ...
                param.put(
                    ...
                    !JobTypeParamSchemas.isUserSetAble(
                        jobInfo.getJobType(), jobInfo.getPkgLang(), s));
                if (value != null) {
  ...
  public Object getTaskInfo(@RequestParam("id") String id) {
    return asNormalResponse(buildTaskGraph(taskManager.getTaskDetailInfo(id)));
  }
  ...
  public Object execTask(@OperationObj @RequestParam("id") String id) {
    ...
    return asNormalResponse(execId);
  }
  ...
  @LoginRequired
  public Object scheduleTask(@OperationObj @RequestParam("id") String id)
  ...
  public Object showExecLog(
      @RequestParam("execId") String id,
      @RequestParam("jobName") String jobName,
      @RequestParam(name = "offset", required = false, defaultValue = "0") int offset,
      @RequestParam(value = "length", required = false, defaultValue = "2000") int length) {
    return asNormalResponse(taskManager.getTaskExecutionLog(id, jobName, offset, length));
  }
  ...
  public Object getTaskName() {
    return asNormalResponse(taskManager.listTaskNames(ContextUtil.getCurrentUser().getTeam()));
  }
  ...
  public Object getJobName(@RequestParam String taskName) {
    return asNormalResponse(taskManager.listJobNames(taskName));
  }

  @ResponseBody
  @GetMapping("task/job/output")
  @LoginRequired
  public Object getJobOutputParams(@RequestParam String taskName, @RequestParam String jobName) {
    return asNormalResponse(taskManager.listJobOutputParams(taskName, jobName));
  }
}
