...
import xinguan.naga.server.var.ParamReference;
import xinguan.naga.server.var.VariableManager;
...
@RestController
@RequestMapping("/naga/v1/param")
@CrossOrigin
public class TaskParamController extends BaseController {

  @Autowired TaskService taskService;
  @Autowired VariableManager variableManager;
  ...
  public Object createVar(@RequestBody @OperationObj ProjectVar var) {
    if (var.getProjectName() == null) {
      return asErrorResponse(ErrorCodes.ERROR_PARAM, "must bind projectName");
    }
    ...
    var.setTrash(false);
    var.setCreateTime(DateUtil.toIntSecond(new Date()));
  ...
  public Object getVar(
      @RequestParam String projectName, @OperationObj @RequestParam String varName) {

    ProjectVar variable = variableManager.getVariable(projectName, varName);
    return asNormalResponse(variable);
  }
  ...
  public Object getJobConfig(
    ...
    for (String key : params.keySet()) {
      ...
      if (reference.isByProject()) {
        ProjectVar variable =
            variableManager.getVariable(reference.getProjectName(), reference.getVarName());
        ...
        if (jobExecData.getParams() != null) {
          ...
          JobInfo jobOfTaskByName =
              taskService.getJobOfTaskByName(taskName, reference.getJobName());
  ...
  @GetMapping("runtime")
  public Object getJobRuntimeConfig(
      @RequestParam("taskName") String taskName,
      @RequestParam("jobName") String jobName,
  ...
  public Object getJobOutputParam(
      @RequestParam("taskName") String taskName,
      @RequestParam("jobName") String jobName,
      @RequestParam("execId") String execId) {
  ...
  public Object getTaskOutputParam(
      @RequestParam("taskName") String taskName, @RequestParam("execId") String execId) {
    TaskWorkFlow taskByName = this.taskService.getTaskByName(taskName);
    List<JobExecData> dataList = this.taskService.getJobExecDataOfTask(taskByName.getId(), execId);
