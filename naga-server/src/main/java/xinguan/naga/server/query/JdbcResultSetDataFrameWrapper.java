...
import xinguan.naga.server.query.cache.CacheThread;
import xinguan.naga.server.query.cache.CacheThreadPool;
import xinguan.naga.server.query.cache.DataFrameCache;
import xinguan.naga.server.query.dataframe.*;
...
public class JdbcResultSetDataFrameWrapper implements ResultSetDataFrameWrapper<ResultSet> {
  ...
  public JdbcResultSetDataFrameWrapper setResources(
      QueryObject queryObject, Statement st, PreparedStatement ps, Connection connection) {
    this.st = st;
    this.ps = ps;
  ...
  public DataFrame wrapData(ResultSet result) {
    try {
      ...
      for (int i = 1; i <= columnCount; i++) {
        ...
        DataType dataType =
            JdbcTypeMapping.getDataType(JDBCType.valueOf(metaData.getColumnType(i)));
  ...
  private void fillDataFrame(ResultSet result, RowSchemaInfo rowSchemaInfo, DataFrameImpl dataFrame)
      throws SQLException {
    ...
    if (result.next()) {
      // todo 获取存储到缓存当中
      String unionStr =
          DataCacheUtil.getUnionStrWithPid(queryObject.getCurrentUser(), queryObject.getSql());
