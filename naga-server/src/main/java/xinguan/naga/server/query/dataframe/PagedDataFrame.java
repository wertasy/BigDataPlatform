package xinguan.naga.server.query.dataframe;
...
import xinguan.naga.server.query.RedisPool;
import lombok.Data;
...
public class PagedDataFrame {
  ...
  public DataFrame executeQuery(String hexKey, int pageIndex, int pageSize) {
    ...
    int start =
        recordStart % DataCacheUtil.PageSize == 0
            ? recordStart / DataCacheUtil.PageSize
            : recordStart / DataCacheUtil.PageSize + 1;
    long stop =
        recordStop % DataCacheUtil.PageSize == 0
    ...
    try {
      ...
      List<byte[]> rangeLists = redisConn.lrange(hexKey.getBytes(), start - 1, stop - 1);
